...
`mamotama-proxy` supports request-time security extensions as compile-time plugins inside `coraza/src/internal/handler`.
...
func (p *sampleRequestSecurityPlugin) Handle(c *gin.Context, ctx *requestSecurityPluginContext) bool {
    if c.GetHeader("X-Mamotama-Sample-Block") == "1" {
